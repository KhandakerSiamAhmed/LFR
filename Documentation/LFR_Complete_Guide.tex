...
\textbf{Crucial Rulebook Compliance}: To strictly adhere to the "No Wireless Communication" rule, the ESP32-S3 Wi-Fi and Bluetooth modems are \textbf{disabled by default} on boot. Wireless telemetry and OTA are exclusively enabled via manual activation inside the "Debug Mode" running off the OLED Menu. In "Run Mode", the network is guaranteed completely off to pass technical inspection seamlessly.
